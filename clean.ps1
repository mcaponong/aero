...
if (Test-Path -Path "C:\csscripts") {
    Write-Host "Folder Exists"	
} else {
	New-Item -Path "c:\" -Name "csscripts" -ItemType "directory"
    Write-Host "Folder Created"
}
	
# List the users in c:\users and export to the local profile for calling later
dir C:\Users | select Name | Export-Csv -Path c:\csscripts\users.csv -NoTypeInformation
$list=Test-Path c:\csscripts\users.csv
""
#########################
"-------------------"
...
if ($list) {
    ...
    Import-CSV -Path c:\csscripts\users.csv -Header Name | foreach {
            ...
            } | Tee-Object -file c:\csscript\mozilla.txt
    Write-Host -ForegroundColor yellow "Clearing Mozilla caches"
...
#            Remove-Item -path "C:\Users\$($_.Name)\AppData\Local\Google\Chrome\User Data\Default\Cookies" -Recurse -Force -EA SilentlyContinue -Verbose
            ...
            } | Tee-Object -file c:\csscript\Chrome.txt
    ...
    Write-Host -ForegroundColor yellow "Clearing Google caches"
    Write-Host -ForegroundColor cyan
    Import-CSV -Path c:\csscripts\users.csv | foreach {
	    ...
	    Remove-Item -path "C:\`$recycle.bin\" -Recurse -Force -EA SilentlyContinue -Verbose
            } | Tee-Object -file c:\csscript\explorer.txt
	Remove-Item -path "c:\csscripts\users.csv" -Recurse -Force -EA SilentlyContinue -Verbose	
    Write-Host -ForegroundColor yellow "Done..."
    ""
